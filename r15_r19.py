...
PHASE = (
    "PHASE 7.5E73B-R10-R4-R8- "
    "ATR CALL PRODUCER LINEAGE AUTHORITY DIAGNOSTIC "
)
...
print(
    "R15_R19_PRE_DIAGNOSTIC_AUTHORITY:",
    "PASS"
    if pre_authority
    else "FAIL",
)
...
for node in ast.walk(definition):
    ...
    atr_entry_assignments.append(record)

    print(
        "atr_entry_EXACT_RHS_RECORD:",
        record,
    )

print(
    "atr_entry_EXACT_ASSIGNMENT_COUNT:",
    len(atr_entry_assignments),
)
...
for assignment in atr_entry_assignments:
    for load_record in assignment[
        ...
        print(
            "atr_entry_RHS_ROOT_RECORD:",
            record,
        )

print(
    "atr_entry_RHS_ROOT_RECORD_COUNT:",
...
def trace_local_name(
    ...
    if key in visited:
        ...
        print(
            "atr_entry_LINEAGE_TERMINAL_RECORD:",
            terminal,
        )
...
print(
    "atr_entry_RECURSIVE_LINEAGE_RECORD_COUNT:",
    len(lineage_records),
)

print(
    "atr_entry_RECURSIVE_TERMINAL_RECORD_COUNT:",
...
for lineage_record in lineage_records:
    ...
    for call_record in assignment[
        ...
        print(
            "atr_entry_CALL_BEARING_LINEAGE_RECORD:",
            record,
        )
...
for record in exact_atr_import_call_records:
    print(
        "atr_entry_EXACT_ATR_IMPORT_CALL_LINEAGE_RECORD:",
        record,
    )

print(
    "atr_entry_CALL_BEARING_LINEAGE_RECORD_COUNT:",
    len(call_bearing_lineage_records),
)

print(
    "atr_entry_EXACT_ATR_IMPORT_CALL_LINEAGE_RECORD_COUNT:",
    len(exact_atr_import_call_records),
)

section("ATR CALL PRODUCER LINEAGE AUTHORITY")

single_assignment_authority = (
    len(atr_entry_assignments) == 1
)
rhs_call_authority = (
    single_assignment_authority
    and atr_entry_assignments[0]["rhs_call_count"] == 1
)

exact_call_lineage_authority = (
    len(rhs_root_records) == 1
    and len(lineage_records) >= 1
    and len(call_bearing_lineage_records) >= 1
)
...
if (
    single_assignment_authority
    and rhs_call_authority
    and exact_call_lineage_authority
):
    classification = (
        "atr_entry_CALL_RHS_TRANSITIVELY_PROJECTS_"
    ...
    classification = (
        "atr_entry_CALL_RHS_TRANSITIVELY_REACHES_"
        "UPDATE_PARAMETER_LINEAGE"
    )

    next_owner = (
        "atr_entry_PARAMETER_TO_LOCAL_PRODUCER_OWNER"
    )
    ...
    next_owner = (
        "atr_entry_IMPORT_BOUND_PRODUCER_SEMANTIC_OWNER"
    )
    ...
    classification = (
        "atr_entry_CALL_RHS_RECURSIVE_LOCAL_"
        "PRODUCER_LINEAGE_UNRESOLVED"
    )

    next_owner = (
        "atr_entry_EXACT_RHS_EXPRESSION_SEMANTIC_OWNER"
    )

    causal_authority = False

print(
    "SINGLE_atr_entry_ASSIGNMENT_AUTHORITY:",
    "PASS"
...
print(
    "atr_entry_EXACT_rhs_call_authority:",
    "PASS"
    if rhs_call_authority
    else "FAIL",
)

print(
    "atr_entry_EXACT_CALL_LINEAGE_AUTHORITY:",
    "PASS"
    if exact_call_lineage_authority
    else "FAIL",
)

print(
    "atr_entry_PARAMETER_TERMINAL_RECORD_COUNT:",
    len(parameter_terminal_records),
)

print(
    "atr_entry_IMPORT_TERMINAL_RECORD_COUNT:",
    len(import_terminal_records),
)

print(
    "atr_entry_UNRESOLVED_TERMINAL_RECORD_COUNT:",
    len(unresolved_terminal_records),
)

print(
    "R15_R19_CLASSIFICATION:",
    classification,
)

print(
    "R15_R19_NEXT_OWNER_CLASSIFICATION:",
    next_owner,
)

print(
    "R15_ATR_IMPORT_PRODUCER_CAUSAL_AUTHORITY"
    "PASS"
    if causal_authority
    else "FAIL",
)
...
print(
    "R15_R19_TARGET_OWNER:",
    "indicators/indicator_engine.py::update_market_state",
)

print(
    "R15_R19_TARGET_LOCAL_SYMBOL:",
    "atr_entry",
)

print(
    "R15_R19_atr_entry_EXACT_ASSIGNMENT_COUNT:",
    len(atr_entry_assignments),
)

print(
    "R15_R19_atr_entry_EXACT_RHS_TYPE:",
    (
        atr_entry_assignments[0]["rhs_type"]
        if len(atr_entry_assignments) == 1
        else None
    ),
)

print(
    "R15_R19_atr_entry_EXACT_RHS_SOURCE:",
    (
        atr_entry_assignments[0]["rhs_source"]
        if len(atr_entry_assignments) == 1
        else None
    ),
)
...
print(
    "R15_R19_atr_entry_EXACT_RHS_CALL_COUNT:",
    (
        atr_entry_assignments[0]["rhs_call_count"]
        if len(atr_entry_assignments) == 1
        else None
    ),
)
...
print(
    "R15_R19_atr_entry_RHS_ROOT_RECORD_COUNT:",
    len(rhs_root_records),
)

print(
    "R15_R19_atr_entry_RECURSIVE_LINEAGE_RECORD_COUNT:",
    len(lineage_records),
)

print(
    "R15_R19_atr_entry_RECURSIVE_TERMINAL_RECORD_COUNT:",
    len(terminal_records),
)

print(
    "R15_R19_atr_entry_CALL_BEARING_LINEAGE_RECORD_COUNT:",
    len(call_bearing_lineage_records),
)

print(
    "R15_R19_atr_entry_EXACT_ATR_IMPORT_CALL_LINEAGE_RECORD_COUNT:",
    len(exact_atr_import_call_records),
)

print(
    "R15_R19_atr_entry_PARAMETER_TERMINAL_RECORD_COUNT:",
    len(parameter_terminal_records),
)

print(
    "R15_R19_atr_entry_IMPORT_TERMINAL_RECORD_COUNT:",
    len(import_terminal_records),
)

print(
    "R15_R19_atr_entry_UNRESOLVED_TERMINAL_RECORD_COUNT:",
    len(unresolved_terminal_records),
)
...
print(
    "R15_atr_entry_CALL_PRODUCER_CAUSAL_AUTHORITY:",
    "PASS"
    if causal_authority
    else "FAIL",
)
...
print(
    "NEXT_ACTION:",
    (
        "REPORT_OUTPUT_ONLY_FOR_atr_entry_NON_CALL_PRODUCER_LINEAGE_REVIEW"
        if final_authority
        else "STOP_AND_REPORT_R15_R19_AUTHORITY_FAILURE"
    ),
)
