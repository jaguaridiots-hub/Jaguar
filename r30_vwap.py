...
"""
R30
VWAP Producer Authority
"""

import ast
from pathlib import Path

TARGET = Path("indicators/vwap.py")
...
if len(functions) != 1:
    raise RuntimeError(
        "Expected exactly one vwap() function"
    )

vwap_function = functions[0]
...
print(
    "VWAP_FUNCTION_LINE:",
    vwap_function.lineno,
)

parameters = [
    arg.arg
    for arg in vwap_function.args.args
]
...
print(
    "VWAP_SIGNATURE_AUTHORITY:",
    "PASS"
    if signature_authority
    else "FAIL",
)
body_length = len(vwap_function.body)

print(
    "VWAP_BODY_STATEMENT_COUNT:",
    body_length,
)

return_nodes = [
    node
    for node in ast.walk(vwap_function)
...
print(
    "VWAP_RETURN_CONTRACT_AUTHORITY:",
    "PASS"
