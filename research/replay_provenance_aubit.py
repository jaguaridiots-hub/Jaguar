...
class ProvenanceBacktestEngine(BacktestEngine):
    def replay(self, state, candles):
        ...
        for index in range(200, len(candles) - 1):
            ...
            # ---- Rest is identical to original replay ----
...
def run_provenance_audit(symbol="GC=F", mode="SCALP"):
    ...
    if len(set(prices)) == 1:
        print("  ❌ OHLCV: constant across all samples – market data is NOT refreshing per candle.")
    else:
        print(f"  ✅ OHLCV: varies across samples (unique={len(set(prices))}) – fresh per candle.")

    lengths = [s['market_current_len'] for s in before_samples]
    if len(set(lengths)) == 1:
        print("  ❌ market_current candle count: constant – slice not being applied.")
    else:
        print(f"  ✅ market_current candle count: varies (min={min(lengths)}, max={max(lengths)}) – slice applied.")

    rsi_vals = [s.get('indicators_rsi') for s in after_samples if s.get('indicators_rsi') is not None]
    if not rsi_vals:
        print("  ⚠️  No RSI indicator found in state.indicators – may be stored elsewhere.")
    elif len(set(rsi_vals)) == 1:
        print("  ❌ RSI: constant across all samples – indicators are NOT recomputed per candle.")
    else:
        print(f"  ✅ RSI: varies across samples (unique={len(set(rsi_vals))}) – indicators recomputed.")

    brain_scores = [s['brain_score'] for s in after_samples if s.get('brain_score') is not None]
    if len(set(brain_scores)) == 1:
        print("  ❌ Brain Score: constant across samples – engines are producing static output.")
    else:
