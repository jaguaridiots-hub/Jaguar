# research/indicator_input_audit.py
import json
import os
import tempfile
...
class IndicatorInputTraceEngine(BacktestEngine):
    def replay(self, state, candles):
        import json
        # Import the actual indicator class
        from indicators.indicator_engine import IndicatorEngine

        log_file = getattr(self, '_log_path', None)

        # Save the original calculate method and patch it
        original_calculate = IndicatorEngine.calculate

        def patched_calculate(candles):
            """Log input size and resulting RSI, then call the original."""
            result = original_calculate(candles)
            if log_file:
                rsi_val = None
                if isinstance(result, dict):
                    rsi_dict = result.get('rsi')
        ...
        try:
            ...
            print(f"Indicator input audit replay complete. Trades: {self.result.total_trades}")
        finally:
...
def run_indicator_input_audit(symbol="GC=F", mode="SCALP"):
    import uuid, copy
    ...
    log_path = os.path.join(tempfile.gettempdir(), f"indicator_input_{symbol}_{mode}.jsonl")
    ...
    register(registry)
    engine = IndicatorInputTraceEngine(registry)
    ...
    print("\n" + "="*70)
    print("  INDICATOR INPUT AUDIT")
    print("="*70)
    print(f"  Logged {len(entries)} calls to IndicatorEngine.calculate()")
    candle_counts = [e['candle_count'] for e in entries]
    rsi_vals = [e['rsi'] for e in entries if e['rsi'] is not None]
    print(f"  Candle counts: {candle_counts}")
    print(f"  Unique candle counts: {len(set(candle_counts))}")
    print(f"  RSI values: {rsi_vals}")
    print(f"  Unique RSI values: {len(set(rsi_vals))}")
    if len(set(rsi_vals)) == 1:
        print("  ❌ RSI constant despite varying input size – calculator may be caching or ignoring input.")
    else:
        print("  ✅ RSI varies with input – dynamic indicators confirmed.")
    print("="*70)
