...
from core.engine_helpers import safe_engine_output
...
class ConfluenceEngine:
    ...
    def evaluate(state):
        ...
        # ---------------- AI ----------------
        brain = safe_engine_output(state, "brain")
        ...
        # ---------------- Multi Timeframe ----------------
        mtf = safe_engine_output(state, "mtf")
        ...
        # ---------------- Order Flow ----------------
        orderflow = safe_engine_output(state, "orderflow")
        ...
        # ---------------- Volume Profile ----------------
        vp = safe_engine_output(state, "volume_profile")
        score += vp.get("score", 0)

        # ---------------- Session ----------------
        session = safe_engine_output(state, "session")
        score += session.get("score", 0)

        # ---------------- Regime ----------------
        regime = safe_engine_output(state, "regime")
        score += regime.get("score", 0)

        # ---------------- Probability ----------------
        probability = safe_engine_output(state, "probability")
        score += probability.get("score", 0)

        # ---------------- Risk ----------------
        risk = safe_engine_output(state, "risk")
