...
engine_classes = [
    node
    for node in classes
    if node.name == "RiskManagerEngine"
]
...
if len(engine_classes) != 1:
    raise RuntimeError(
        "Expected exactly one RiskManagerEngine"
    )
...
print("RUN_METHOD_COUNT:", len(run_methods))

if len(run_methods) != 1:
    raise RuntimeError("Expected exactly one run() method")

run_method = run_methods[0]

print("RUN_METHOD_LINE:", run_method.lineno)

parameters = [arg.arg for arg in run_method.args.args]

print("RUN_PARAMETERS:", parameters)
print("RUN_PARAMETER_COUNT:", len(parameters))
